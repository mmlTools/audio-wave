...
static bool square_style_modified(obs_properties_t *props, obs_property_t *, obs_data_t *settings)
{
	const char *style_id = obs_data_get_string(settings, SQUARE_PROP_STYLE);
	const bool is_rays = (style_id && strcmp(style_id, "rays") == 0);

	obs_property_t *mirror = obs_properties_get(props, SQUARE_PROP_MIRROR);
	if (mirror)
...
{
	obs_property_t *style = obs_properties_add_list(props, SQUARE_PROP_STYLE, "Style", OBS_COMBO_TYPE_LIST,
	...
	obs_property_set_modified_callback(style, square_style_modified);

	obs_property_t *mirror = obs_properties_add_bool(props, SQUARE_PROP_MIRROR, "Double-sided rays");
	obs_property_set_visible(mirror, false);
	obs_properties_add_int_slider(props, P_DENSITY, "Shape Density (%)", 10, 300, 5);

}

static void square_theme_update(audio_wave_source *s, obs_data_t *settings)
{
	if (!s || !settings)
		return;
	...
	s->mirror = (strcmp(style_id, "rays") == 0) ? obs_data_get_bool(settings, SQUARE_PROP_MIRROR) : false;
}

// ─────────────────────────────────────────────
// Geometry helpers for square outline
// ─────────────────────────────────────────────
...
static void build_square_vertices(const audio_wave_source *s, std::vector<square_vertex> &verts)
{
	verts.clear();
	...
	const float half = std::max(1.0f, std::min(w, h) * 0.5f - 1.0f);
...
static void sample_square_outline(const audio_wave_source *s, const std::vector<square_vertex> &verts, float u,
				  float &x, float &y, float &nx, float &ny)
{
	...
	const square_vertex &v0 = verts[i];
	const square_vertex &v1 = verts[(i + 1) % N];
...
static void draw_square_orbit(audio_wave_source *s, gs_eparam_t *color_param)
{
	...
	std::vector<square_vertex> verts;
	build_square_vertices(s, verts);
	if (verts.empty())
		return;
	...
	for (uint32_t i = 0; i < segments; ++i) {
		...
		sample_square_outline(s, verts, u, x, y, nx_unused, ny_unused);
		base_x[i] = x;
		base_y[i] = y;
...
static void draw_square_filled(audio_wave_source *s, gs_eparam_t *color_param)
{
    const size_t frames = s->wave.size();
    if (!frames || s->width <= 0 || s->height <= 0)
        return;
    ...
    const float max_len = std::min(w, h) * 0.22f;
    ...
    uint32_t segments = (uint32_t)std::clamp(density_raw * 4.0f, 48.0f, 2048.0f);
    if (!segments)
        return;
    ...
    for (uint32_t i = 0; i < segments; ++i) {
        ...
        float x, y, nx, ny;
        sample_square_outline(s, verts, u, x, y, nx, ny);
        base_x[i] = x;
        base_y[i] = y;
    ...
    float prev = amp[0];
    amp_smooth[0] = prev;
    const float a1 = 0.16f;
    for (uint32_t i = 1; i < segments; ++i) {
        prev = prev + a1 * (amp[i] - prev);
        amp_smooth[i] = prev;
    }
    prev = amp_smooth[0];
    for (uint32_t i = 1; i < segments; ++i) {
        prev = prev + 0.12f * (amp_smooth[i] - prev);
        amp_smooth[i] = prev;
    }

    auto point_at = [&](uint32_t i, float &x2, float &y2) {
        const float v_raw = amp_smooth[i];
        const float v = std::clamp(audio_wave_apply_curve(s, v_raw), 0.0f, 1.0f);
        const float len = v * max_len;
    ...
    if (color_param)
        audio_wave_set_solid_color(color_param, s->gradient_enabled ? aw_gradient_color_at(s, 0.5f) : s->color);

    gs_render_start(true);
    for (uint32_t i = 0; i < segments; ++i) {
        float x1, y1, x2, y2;
        point_at(i, x1, y1);
        point_at((i + 1) % segments, x2, y2);
...
static void draw_square_rays(audio_wave_source *s, gs_eparam_t *color_param)
{
	const size_t frames = s->wave.size();
	if (!frames || s->width <= 0 || s->height <= 0)
		return;
	...
	for (uint32_t i = 0; i < segments; ++i) {
		...
		float x, y, nxx, nyy;
		sample_square_outline(s, verts, u, x, y, nxx, nyy);
		base_x[i] = x;
		base_y[i] = y;
		nx[i] = nxx;
		ny[i] = nyy;
...
static void square_theme_draw(audio_wave_source *s, gs_eparam_t *color_param)
{
	...
	if (s->theme_style_id == std::string("rays")) {
		draw_square_rays(s, color_param);
	} else {
		draw_square_orbit(s, color_param);
	}

	gs_matrix_pop();
}

static void square_theme_destroy_data(audio_wave_source *s)
{
	UNUSED_PARAMETER(s);
}

static const audio_wave_theme k_square_theme = {
	k_theme_id_square,   k_theme_name_square, square_theme_add_properties,
	square_theme_update, square_theme_draw,   square_theme_destroy_data,
};

void audio_wave_register_square_theme()
{
	audio_wave_register_theme(&k_square_theme);
}