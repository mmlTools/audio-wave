...
static void draw_circle_filled(audio_wave_source *s, gs_eparam_t *color_param)
{
    const size_t frames = s->wave.size();
    if (!frames || s->width <= 0 || s->height <= 0)
        return;
    ...
    uint32_t segments = (uint32_t)std::clamp(density_raw * 6.0f, 48.0f, 2048.0f);
    if (segments < 8)
        return;
    ...
    // spatial smoothing (fluid silhouette)
    float prev = amp[0];
    amp_smooth[0] = prev;
    ...
    auto point_at = [&](uint32_t i, float &x, float &y) {
        const float u = (float)i / (float)segments;
        const float a_raw = amp_smooth[i];
        const float v = std::clamp(audio_wave_apply_curve(s, a_raw), 0.0f, 1.0f);
        const float r = R_base + v * L_max;
