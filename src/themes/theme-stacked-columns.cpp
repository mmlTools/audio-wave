...
static void draw(audio_wave_source *s, gs_eparam_t *color_param)
{
	...
	const bool compact = (s->theme_style_id == "compact");
	...
	const float block_w = col_w * (compact ? 0.92f : 0.72f);
	...
	const float gap_scale = compact ? 0.55f : 1.0f;
	const float gap = std::clamp(d->gap_ratio, 0.0f, 0.45f) * block_h * gap_scale;
	...
	for (int c = 0; c < cols; ++c) {
		const float t_data = (cols <= 1) ? 0.0f : ((float)c / (float)(cols - 1));

		// Mirror by flipping draw index (geometry). This guarantees a visible mirror.
		const int c_draw = d->mirror ? (cols - 1 - c) : c;
		const float t_draw = (cols <= 1) ? 0.0f : ((float)c_draw / (float)(cols - 1));

		float amp = sample_wave(s, t_data);
		...
		const float x0 = (float)c_draw * col_w + x_pad;
		const float x1 = x0 + block_w;

		const int b = bin_for_t(t_draw);
